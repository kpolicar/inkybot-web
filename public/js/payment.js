...
var refreshPrice = function refreshPrice() {
  ...
  axios.get("".concat(priceFetchUrl, "?plan=").concat(this.plan, "&quantity=").concat(this.quantity, "&queue=").concat(this.queue)).then(function (result) {
    _this.price = result.data['amount'] / 100;
  })["finally"](function () {
    _this.priceRefreshRequest = false;
  });
